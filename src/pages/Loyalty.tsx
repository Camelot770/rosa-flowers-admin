...
import { Loader2, X, History, Plus, Minus } from 'lucide-react';
...
export default function Loyalty() {
  ...
  const handleAdjust = async (e: FormEvent) => {
    ...
    try {
      await api.post(`/loyalty/${userId}/adjust`, {
        amount: Number(adjustAmount),
        description: adjustDescription.trim() || undefined,
      });

      // Refresh user list and history
      await fetchUsers();
      const { data } = await api.get(`/loyalty/${userId}`);
      setHistory(Array.isArray(data) ? data : data.history || []);

      // Update selectedUser points locally
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Программа лояльности</h1>
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  Баллы: <span className="font-semibold text-primary">{selectedUser.bonusPoints}</span>
                </p>
              </div>
              <button
                onClick={closeModal}
                      ...
                      >
                        <div>
                          <p className="text-sm text-gray-700">
                            {entry.description || 'Корректировка'}
                          </p>
                          {dateStr && (
